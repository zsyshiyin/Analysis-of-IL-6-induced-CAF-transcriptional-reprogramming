...
setwd("D:\\zsy\\SX\\MRC-5-Z-GSEA\\MRC-5-qinxiqianyi-xin")  # 请替换为您的实际工作路径

# 设置您的数据文件路径（可以使用相对路径或绝对路径）
# 如果数据文件在工作目录下，可以直接使用文件名
data_file_path <- "GSEAzzt.csv"  # 修改为相对路径
...
gradient_colors <- list(
  ...
  # (20S) G-Rh2_vs_IL-6 的配色
  "(20S) G-Rh2_vs_IL-6" = list(
...
prepare_gsea_data <- function(df) {
  ...
  # 创建柱条宽度映射 - q值越小，柱条越粗
  # 调整宽度映射，确保q值<0.7都能显示明显的条带，宽度范围0.15-0.75
  df$bar_width <- 0.15 + (1 - pmin(df$FDR, 0.7)/0.7) * 0.6  # 宽度范围：0.15-0.75
  
  # 创建柱条透明度映射 - p值越小，柱条越不透明
  # 修正透明度映射：p值=0时alpha=1.0（完全不透明），p值=0.2时alpha=0.4
  df$bar_alpha <- 1.0 - (pmin(df$NOM_pVal, 0.5)/0.5) * 0.8  # 透明度范围：0.2-1.0
