...
setwd("D:\\zsy\\SX\\MRC-5-Z-GSEA\\matrix-MDA-231-DMEM\\z-GSEA")  # 请替换为您的实际工作路径

# 设置您的数据文件路径（可以使用相对路径或绝对路径）
data_file_path <- "GSEAzzt-matrix-MDA231-DMEM.csv"  # 修改为相对路径

# 渐变颜色设置 - 根据NES正负值设置单个比较组的渐变配色
gradient_colors <- list(
  # NF-CM_vs_MDA-231-CM 的配色
  "NF-CM_vs_MDA-231-CM" = list(
    positive = c(left = "#2AA7DE", right = "#25377F"),  # 红色渐变（正值）
    negative = c(left = "#25377F", right = "#2AA7DE")   # 蓝色渐变（负值）
  )
)
...
font_sizes <- list(
  ...
  axis_text = 14,
  strip_text = 16,
  pathway_text = 24  # 通路名称增大2个字号（从20增大到24）
)
...
x_axis_physical_width <- 7.8  # 直接设置横坐标轴的物理宽度，增大此值使横坐标轴变宽
y_label_width <- 3.5        # 左侧Y轴标签区域固定宽度（一般不需要修改）

# 输出文件设置（使用相对路径，将保存在工作目录下）
main_output_file <- "GSEA_comparison_gradient_bar_plot_NF-CM_vs_MDA-231-CM.png"
main_output_pdf <- "GSEA_comparison_gradient_bar_plot_NF-CM_vs_MDA-231-CM.pdf"
legend_output_file <- "GSEA_comparison_gradient_bar_plot_legends_NF-CM_vs_MDA-231-CM.png"
legend_output_pdf <- "GSEA_comparison_gradient_bar_plot_legends_NF-CM_vs_MDA-231-CM.pdf"
...
#' @return 换行后的文本
wrap_pathway_text <- function(text, max_words = 4) {
  # 如果文本为空，直接返回
...
process_pathway_text_wrap <- function(df) {
  df <- df %>%
    mutate(
      # 为每条通路创建换行后的文本
      Pathway_wrapped = sapply(Pathway, wrap_pathway_text, max_words = 4)
    )
  return(df)
}
...
prepare_gsea_data <- function(df) {
  ...
  # 创建柱条宽度映射 - q值越小，柱条越粗
  df$bar_width <- 0.25 + (1 - pmin(df$FDR, 0.5)/0.5) * 0.5  # 宽度范围：0.25-0.75
  
  # 创建柱条透明度映射 - p值越小，柱条越不透明
  df$bar_alpha <- 1.0 - (pmin(df$NOM_pVal, 0.2)/0.2) * 0.6  # 透明度范围：0.4-1.0
  ...
  for (i in 1:nrow(df)) {
    ...
    for (j in 1:n_points) {
      ...
      gradient_data <- rbind(gradient_data, data.frame(
        Pathway = row$Pathway,
        Pathway_wrapped = row$Pathway_wrapped,
...
plot_gsea_comparison_no_legend <- function(df) {
  ...
  p <- ggplot() +
    ...
    scale_y_continuous(
      ...
      labels = y_labels[pathway_levels]
    ) +
...
create_comparison_legend <- function() {
  ...
  # 使用单个比较组
  comparison <- "NF-CM_vs_MDA-231-CM"
  ...
  for (j in 1:10) {
    ...
    # 计算渐变颜色
    r1 <- col2rgb(colors$positive["left"])[1]
    g1 <- col2rgb(colors$positive["left"])[2]
    b1 <- col2rgb(colors$positive["left"])[3]
    
    r2 <- col2rgb(colors$positive["right"])[1]
    g2 <- col2rgb(colors$positive["right"])[2]
    b2 <- col2rgb(colors$positive["right"])[3]
  ...
  for (j in 1:10) {
    ...
    # 计算渐变颜色
    r1 <- col2rgb(colors$negative["left"])[1]
    g1 <- col2rgb(colors$negative["left"])[2]
    b1 <- col2rgb(colors$negative["left"])[3]
    
    r2 <- col2rgb(colors$negative["right"])[1]
    g2 <- col2rgb(colors$negative["right"])[2]
    b2 <- col2rgb(colors$negative["right"])[3]
  ...
  label_data <- data.frame(
    Type = c("Positive (NES > 0)", "Negative (NES < 0)"),
    x = 0.92,
    y = c(1.76, 1.16)
  )
  ...
  p <- ggplot() +
    ...
    # 添加比较组标题
    annotate("text", x = 0.86, y = 2.6, label = "NF-CM_vs_MDA-231-CM", 
             hjust = 0, size = 4, fontface = "bold") +
    xlim(0.5, 2.0) +
    ylim(0.5, 2.8) +
    labs(title = "比较组及NES方向") +
    theme_void() +
    theme(
...
# 筛选只保留指定的比较组
cat("\n正在筛选比较组: NF-CM_vs_MDA-231-CM\n")
gsea_data <- gsea_data[gsea_data$Comparison == "NF-CM_vs_MDA-231-CM", ]

# 检查筛选后的数据
if (nrow(gsea_data) == 0) {
  stop("错误: 筛选后没有数据，请确认数据文件中包含 'NF-CM_vs_MDA-231-CM' 比较组")
}

cat("筛选完成，剩余", nrow(gsea_data), "行数据\n")
