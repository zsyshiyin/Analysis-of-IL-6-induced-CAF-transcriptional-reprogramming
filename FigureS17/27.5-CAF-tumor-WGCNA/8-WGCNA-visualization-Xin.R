...
# ========== 新增：自定义分组排列顺序 ==========
# 在这里设置你想要的组别顺序（从左到右显示）
# 示例1：默认顺序
# custom_group_order <- c("DMEM", "Control", "I6", "R2")

# 示例2：自定义顺序（根据你的需求修改）
custom_group_order <- c("DMEM", "Control", "I6", "R2")  # 修改这里即可

# 示例3：如果只想显示部分组别
# custom_group_order <- c("I6", "Control", "R2")

# 是否只显示custom_group_order中指定的组别（TRUE=只显示指定组，FALSE=显示所有组）
filter_groups <- TRUE
...
# 模块顺序自定义（可选）
# 示例：custom_module_order <- c("turquoise", "blue", "brown", "yellow", "green", "red", "black", "pink")
custom_module_order <- NULL  # NULL表示自动按大小排序
...
group_mapping <- list("DMEM" = "DMEM", "Con" = "Control",  "I6" = "I6", "R2" = "R2")
...
if (filter_groups && !is.null(custom_group_order)) {
  # 只保留自定义顺序中存在的组别
  groups_to_keep <- intersect(custom_group_order, all_groups)
  sample_groups <- sample_groups[sample_groups$Group %in% groups_to_keep, ]
  groups <- groups_to_keep
  cat("按自定义顺序筛选后的分组:", paste(groups, collapse=", "), "\n")
} else if (!is.null(custom_group_order)) {
  # 保留所有组，但按自定义顺序排列
  groups <- custom_group_order[custom_group_order %in% all_groups]
  cat("按自定义顺序排列的分组:", paste(groups, collapse=", "), "\n")
} else {
  groups <- all_groups
  cat("原始分组顺序:", paste(groups, collapse=", "), "\n")
}

# 重新将Group转换为因子，应用自定义顺序
sample_groups$Group <- factor(sample_groups$Group, levels = groups)
...
cat("性状矩阵列名（按自定义顺序）:", paste(colnames(datTraits), collapse=", "), "\n")
...
# ========== 自定义热图颜色 ==========
module_trait_colors <- c("#4A60B5", "white", "#E04A6E")
...
# ========== 新增：按自定义顺序排列模块 ==========
# 如果有自定义模块顺序，重新排列
if (!is.null(custom_module_order)) {
  available_modules <- intersect(custom_module_order, rownames(moduleTraitCor))
  other_modules <- setdiff(rownames(moduleTraitCor), available_modules)
  module_order <- c(available_modules, other_modules)
  moduleTraitCor <- moduleTraitCor[module_order, , drop = FALSE]
  textMatrix <- textMatrix[module_order, , drop = FALSE]
  cat("按自定义顺序排列模块\n")
}
...
if (!is.null(enrichment_results) && nrow(enrichment_results) > 0) {
  ...
  # ========== 新增：按自定义模块顺序排列 ==========
  if (!is.null(custom_module_order)) {
    available_modules <- intersect(custom_module_order, all_modules)
    other_modules <- setdiff(all_modules, available_modules)
    module_order <- c(available_modules, other_modules)
    ...
    module_order <- names(sort(module_sizes, decreasing = TRUE))
    module_enrichment_summary <- module_enrichment_summary %>%
...
cat("\n二、分组顺序设置\n")
cat("--------------------------------------------------------------------------------\n")
cat("当前分组顺序:", paste(groups, collapse=" → "), "\n")
if (filter_groups && !is.null(custom_group_order)) {
  cat("已应用分组筛选，只显示指定组别\n")
}
