...
# 工作路径与输出目录
# ============================================================================
work_dir <- "D:/zsy/SX/Fomal-final/5.5-CIPS-TF"
setwd(work_dir)
...
# ---- 主图表达热图 (Figure 2: DoRothEA ≥ 4, 无文献标注) ----
...
fig2_annotation_colors <- list(
  Group = c("Con" = "#25377F", "I6" = "#C60036", "R2" = "#E4945A")
)

# ---- 主图活性热图 (Figure 3: DoRothEA ≥ 4, 无文献标注) ----
...
MIN_DOROTHEA_FOR_MAIN <- 4  # 主图最低DoRothEA验证靶基因数
...
GROUP_MAPPING <- list("Con" = "Con", "I6" = "I6", "R2" = "R2")
GROUP_ORDER <- c("Con", "I6", "R2")
GROUP_COLORS <- c("Con" = "#25377F", "I6" = "#C60036", "R2" = "#E4945A")
...
draw_barplot <- function(display_df, fig_params, output_prefix) {
  ...
  display_df <- display_df %>%
    mutate(
      TF_label = TF,
      TF_display = ifelse(Literature_Known, 
                          paste0("bold('", TF, "')"), 
                          TF)
    )
  ...
  # 文献已知TF的Y轴标签加粗
  # 构建加粗/不加粗的映射
  tf_face_map <- display_df %>%
    mutate(TF = factor(TF, levels = rev(TF))) %>%
    dplyr::select(TF, Literature_Known)
...
draw_heatmap <- function(display_df, matrix_to_plot, fig_params, output_prefix, plot_type) {
  ...
  plot_data <- matrix_to_plot[display_tfs, , drop = FALSE]
  ...
  # 只有分组标注，无文献标注
  anno_col <- data.frame(Group = sample_groups$Group, row.names = sample_groups$SampleID)
...
cat("--------------------------------------------------------------------------------\n")
cat("  Figure0 - 阈值优化图\n")
cat("  Figure1_Main_barplot - 主图条形图 (★ = CAF文献已知)\n")
cat("  Figure2_Main_expression_heatmap - 主图表达热图\n")
cat("  Figure3_Main_activity_heatmap - 主图活性热图\n")
cat("  FigureS1_Supplementary_barplot - 补充条形图 (★ = CAF文献已知)\n")
cat("  FigureS2_Supplementary_expression_heatmap - 补充表达热图\n")
cat("  FigureS3_Supplementary_activity_heatmap - 补充活性热图\n")
cat("  Table_S1-S4 - 完整数据表\n")
